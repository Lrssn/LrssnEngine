...
namespace LrssnEngine {
	...
	namespace Utils {
		...
		static GLenum LrssnEngineFBTextureFormatToGL(FramebufferTextureFormat format) {
			switch (format) {
	...
	void OpenGLFramebuffer::Invalidate() {
		...
		if (mColorAttachments.size() > 1) {
			LE_CORE_ASSERT(mColorAttachments.size() <= 4);
	...
	int OpenGLFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y) {
		LE_CORE_ASSERT(attachmentIndex < mColorAttachments.size());
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value) 	{
		...
		glClearTexImage(mColorAttachments[attachmentIndex], 0,
			Utils::LrssnEngineFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
}