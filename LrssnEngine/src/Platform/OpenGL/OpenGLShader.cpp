...
#include "LrssnEngine/Core/Timer.h"
...
namespace LrssnEngine {
	...
	OpenGLShader::OpenGLShader(const std::string& filepath) 
		...
		auto shaderSources = PreProcess(source);
		{
			...
			CreateProgram();
			LE_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source) 	{
		...
		while (pos != std::string::npos) 		{
			...
			LE_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");
		...
		for (auto&& [stage, source] : shaderSources) {
			...
			if (in.is_open()) {
				...
				if (module.GetCompilationStatus() != shaderc_compilation_status_success) {
					LE_CORE_ERROR(module.GetErrorMessage());
					LE_CORE_ASSERT(false);
				}
	...
	void OpenGLShader::CreateProgram() {
		...
		if (isLinked == GL_FALSE) {
			...
			LE_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData) {
		...
		LE_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		LE_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		LE_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		LE_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers) {
			...
			LE_CORE_TRACE("  {0}", resource.name);
			LE_CORE_TRACE("    Size = {0}", bufferSize);
			LE_CORE_TRACE("    Binding = {0}", binding);
			LE_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
